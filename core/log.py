...
def _log(config, step_count, log_data, model, replay_buffer, lr, shared_storage, summary_writer, vis_result):
    ...
    total_loss, weighted_loss, loss, reg_loss, policy_loss, value_prefix_loss, value_loss, consistency_loss, mlr_loss = loss_data
    if vis_result:
    ...
    _msg = '#{:<10} Total Loss: {:<8.3f} [weighted Loss:{:<8.3f} Policy Loss: {:<8.3f} Value Loss: {:<8.3f} ' \
           'Reward Sum Loss: {:<8.3f} Consistency Loss: {:<8.3f} MLR Loss: {:<8.3f}] ' \
           'Replay Episodes Collected: {:<10d} Buffer Size: {:<10d} Transition Number: {:<8.3f}k ' \
           'Batch Size: {:<10d} Lr: {:<8.3f}'
    _msg = _msg.format(step_count, total_loss, weighted_loss, policy_loss, value_loss, value_prefix_loss, consistency_loss, mlr_loss,
    ...
    if summary_writer is not None:
        ...
        summary_writer.add_scalar('{}/mlr_loss'.format(tag), mlr_loss, step_count)
